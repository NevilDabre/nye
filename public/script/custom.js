...
setTimeout(function () {
  ...
  const crysanthemumShell = (size = 1) => {
    ...
    const secondColor = singleColor && (Math.random() < 0.2 || color === COLOR.White) ? pistilColor ||
      randomColor({
        notColor: color,
        limitWhite: true
      }) : null;
  ...
  $("#goButton").click(function () {
    if ($.trim($('#searchcontent').val()) == '') {
      ...
      $("#name").text($('#searchcontent').val() + ' wishes you,')
      $(".searchbar").addClass("hide");
      $(".searchbar").removeClass("error");
      $(".whatsappbar").removeClass("hide");
    }
  })

  var audio = new Audio('./wwc.mp3');
  audio.loop = true;

  $(window).on("click mouseleave mouseenter touchstart mouseout mouseover", function () {
  ...
  $("#muteBgAudio").click(function () {
    ...
    $("#muteBgAudio").addClass('hide');
    $('#startBgAudio').removeClass('hide');
  })

  $("#startBgAudio").click(function () {
    var playPromise = audio.play();
    ...
    $("#startBgAudio").addClass('hide');
    $('#muteBgAudio').removeClass('hide');
  })
